#Fitting a Bayesian nonparametric model for a single data of the type Example 3.
...
bnp_mod <- function(data){
  ...
  DD3 <- cbind(y3[,"D1"],0,0,0)
  DD4 <- cbind(0,y4[,"D2"],0,0)
  DD5 <- cbind(0,y5[,"D2"],0,y5[,"D4"])
  DD6 <- cbind(0,y6[,"D2"],0,0)
  ...
  DD6[,c(1,3,4)] <- cbind(rtruncnorm(n6,mean(DD6[,2]),0.1,a=0,b=1),0,
                          rtruncnorm(n6,mean(DD6[,2]),0.5,a=0,b=1))
  ...
  #Initial values for the parameters of the marginals of the compliances
  para.dd1 <- matrix(nrow=mcmc,ncol=K+K+2+K+5+1+n)
  para.dd1[1,] <-para.dd1[2,] <- c(rep(1/K,K),rep(coefficients(lm(D[1:(n1+n2+n3),1]~rbind(xx1,xx2,xx3)[,1]+
                                                                    rbind(xx1,xx2,xx3)[,2]))[1],K),
                                   coefficients(lm(D[1:(n1+n2+n3),1]~rbind(xx1,xx2,xx3)[,1]+
                                                     rbind(xx1,xx2,xx3)[,2]))[-1],
                                   rep(0.1,K),2,2,2,mean(D[1:(n1+n2+n3),1]),500,1, 
                                   sample(1:K,n, replace=TRUE))
  
  para.dd2 <- matrix(nrow=mcmc,ncol=K+K+2+K+5+1+n)
  para.dd2[1,] <-para.dd2[2,] <- c(rep(1/K,K),rep(coefficients(lm(D[(n1+n2+n3+1):n,2]~rbind(xx4,xx5,xx6)[,1]+
                                                                    rbind(xx4,xx5,xx6)[,2]))[1],K),
                                   coefficients(lm(D[(n1+n2+n3+1):n,2]~rbind(xx4,xx5,xx6)[,1]+
                                                     rbind(xx4,xx5,xx6)[,2]))[-1],
                                   rep(0.1,K),2,2,2,mean(D[(n1+n2+n3+1):n,2]),5,1, 
                                   sample(1:K,n, replace=TRUE))
  ...
  para.dd3[1,] <-para.dd3[2,] <- c(rep(1/K,K),rep(coefficients(lm(D[(n1+1):(n1+n2),3]~xx2[,1]+xx2[,2]+xx2[,3]+
                                   ...
                                   coefficients(lm(D[(n1+1):(n1+n2),3]~xx2[,1]+xx2[,2]+xx2[,3]
                                                   +D[(n1+1):(n1+n2),1]))[-1],
                                   rep(0.1,K),2,2,2,mean(DD2[,3]),5,1, 
                                   sample(1:K, n, replace=TRUE))
  ...
  para.dd4[1,] <-para.dd4[2,] <- c(rep(1/K,K),rep(coefficients(lm(D[c(n1+n2+n3+n4+1):(n-n6),4]~xx5[,1]+xx5[,2]+xx5[,3]
                                   ...
                                   coefficients(lm(D[(n1+n2+n3+n4+1):(n-n6),4]~x1[(n1+n2+n3+n4+1):(n-n6)]
                                                   +x2[(n1+n2+n3+n4+1):(n-n6)]+x3[(n1+n2+n3+n4+1):(n-n6)]
                                                   +D[(n1+n2+n3+n4+1):(n-n6),2]))[-1],
                                   rep(0.1,K),2,2,2,mean(DD5[,4]),5,1, 
                                   sample(1:K, n, replace=TRUE))
  ...
  cov.d11 <- diag(vcov(lm(D[1:(n1+n2+n3),1]~rbind(xx1,xx2,xx3)[,1]+
                            rbind(xx1,xx2,xx3)[,2]))[1,1],K)/4
  cov.d12 <- diag(vcov(lm(D[(n1+n2+n3+1):n,2]~rbind(xx4,xx5,xx6)[,1]+
                            rbind(xx4,xx5,xx6)[,2]))[1,1],K)/4
  cov.d13 <- diag(vcov(lm(DD2[,3]~xx2[,1]+xx2[,2]+xx2[,3]+DD2[,1]))[1,1],K)/4
  cov.d14 <- diag(vcov(lm(DD5[,4]~xx5[,1]+xx5[,2]+xx5[,3]+DD5[,2]))[1,1],K)/4
  ...
  w1 <- cbind(yy1,h1[,1:2],xx1[,-3]); wbar1 <- apply(w1,2,mean)
  w2 <- cbind(yy2,h2[,1:3],xx2[,-3]); wbar2 <- apply(w2,2,mean)
  w3 <- cbind(yy3,h3[,1:3],xx3[,-3]); wbar3 <- apply(w3,2,mean)
  w4 <- cbind(yy4,h4[,1:2],xx4[,-3]); wbar4 <- apply(w4,2,mean)
  w5 <- cbind(yy5,h5[,c(1,2,4)],xx5[,-3]); wbar5 <- apply(w5,2,mean)
  w6 <- cbind(yy6,h6[,c(1,2,4)],xx6[,-3]); wbar6 <- apply(w6,2,mean)
  
  # Fitting DPM for the joint distribution of (Y,H,X0)
  g0priors1 <- list(mu0=wbar1,Lambda=10*diag(5),kappa0=5,nu=5)
  dp1 <- Fit(DirichletProcessMvnormal(y=w1,g0Priors = g0priors1),100)
  
  g0priors2 <- list(mu0=wbar2,Lambda=10*diag(6),kappa0=6,nu=6)
  dp2 <- Fit(DirichletProcessMvnormal(y=w2,g0Priors = g0priors2),100)
  
  g0priors3 <- list(mu0=wbar3,Lambda=10*diag(6),kappa0=6,nu=6)
  dp3 <- Fit(DirichletProcessMvnormal(y=w3,g0Priors = g0priors3),100)
  
  g0priors4 <- list(mu0=wbar4,Lambda=10*diag(5),kappa0=5,nu=5)
  dp4 <- Fit(DirichletProcessMvnormal(y=w4,g0Priors = g0priors4),100)
  
  g0priors5 <- list(mu0=wbar5,Lambda=10*diag(6),kappa0=6,nu=6)
  dp5 <- Fit(DirichletProcessMvnormal(y=w5,g0Priors = g0priors5),100)
  
  g0priors6 <- list(mu0=wbar6,Lambda=10*diag(6),kappa0=6,nu=6)
  ...
  for(t in 3:mcmc){
    ...
    if(t %in% SEQ){
      ...
      den<-sapply(1:length(pi1), function(i) dmnorm(cbind(h1[,1:2],xx1[,-3]), 
                                                    Mu1[[i]][-c(1)], 
                                                    Sigma1[[i]][-c(1),
                                                                -c(1)])*pi1[i])/
        rowSums(sapply(1:length(pi1), function(i) dmnorm(cbind(h1[,1:2],xx1[,-3]), 
                                                         Mu1[[i]][-c(1)], 
      ...
      y_1[[cc]]<-rowSums(sapply(1:length(pi1), function(i) CondMVN(mean=Mu1[[i]] , sigma=Sigma1[[i]],
                                                                   dependent=c(1), given=c(2:5),X.given=
                                                                     cbind(h1[,1:2],xx1[,-3]))$condMean)*(den1))
      
      den<-sapply(1:length(pi2), function(i) dmnorm(cbind(h2[,1:3],xx2[,-3]), 
                                                    Mu2[[i]][-c(1)], 
                                                    Sigma2[[i]][-c(1),
                                                                -c(1)])*pi2[i])/
        rowSums(sapply(1:length(pi2), function(i) dmnorm(cbind(h2[,1:3],xx2[,-3]), 
                                                         Mu2[[i]][-c(1)], 
      ...
      y_2[[cc]]<-rowSums(sapply(1:length(pi2), function(i) CondMVN(mean=Mu2[[i]] , sigma=Sigma2[[i]],
                                                                   dependent=c(1), given=c(2:6),X.given=
                                                                     cbind(h2[,1:3],xx2[,-3]))$condMean)*(den1))
      
      den<-sapply(1:length(pi3), function(i) dmnorm(cbind(h3[,1:3],xx3[,-3]), 
                                                    Mu3[[i]][-c(1)], 
                                                    Sigma3[[i]][-c(1),
                                                                -c(1)])*pi3[i])/
        rowSums(sapply(1:length(pi3), function(i) dmnorm(cbind(h3[,1:3],xx3[,-3]), 
                                                         Mu3[[i]][-c(1)], 
                                                         Sigma3[[i]][-c(1),-c(1)])*pi3[i]))
      den1<-ifelse(is.na(den), 0, den)
      y_3[[cc]]<-rowSums(sapply(1:length(pi3), function(i) CondMVN(mean=Mu3[[i]] , sigma=Sigma3[[i]],
                                                                   dependent=c(1), given=c(2:6),X.given=
                                                                     cbind(h3[,1:3],xx3[,-3]))$condMean)*(den1))
      
      den<-sapply(1:length(pi4), function(i) dmnorm(cbind(h4[,1:2],xx4[,-3]), 
                                                    Mu4[[i]][-c(1)], 
                                                    Sigma4[[i]][-c(1),
                                                                -c(1)])*pi4[i])/
        rowSums(sapply(1:length(pi4), function(i) dmnorm(cbind(h4[,1:2],xx4[,-3]), 
                                                         Mu4[[i]][-c(1)], 
                                                         Sigma4[[i]][-c(1),-c(1)])*pi4[i]))
      den1<-ifelse(is.na(den), 0, den)
      y_4[[cc]]<-rowSums(sapply(1:length(pi4), function(i) CondMVN(mean=Mu4[[i]] , sigma=Sigma4[[i]],
                                                                   dependent=c(1), given=c(2:5),X.given=
                                                                     cbind(h4[,1:2],xx4[,-3]))$condMean)*(den1))
      
      den<-sapply(1:length(pi5), function(i) dmnorm(cbind(h5[,c(1,2,4)],xx5[,-3]), 
                                                    Mu5[[i]][-c(1)], 
                                                    Sigma5[[i]][-c(1),
                                                                -c(1)])*pi5[i])/
        rowSums(sapply(1:length(pi5), function(i) dmnorm(cbind(h5[,c(1,2,4)],xx5[,-3]), 
                                                         Mu5[[i]][-c(1)], 
                                                         Sigma5[[i]][-c(1),-c(1)])*pi5[i]))
      den1<-ifelse(is.na(den), 0, den)
      y_5[[cc]]<-rowSums(sapply(1:length(pi5), function(i) CondMVN(mean=Mu5[[i]] , sigma=Sigma5[[i]],
                                                                   dependent=c(1), given=c(2:6),X.given=
                                                                     cbind(h5[,c(1,2,4)],xx5[,-3]))$condMean)*(den1))
      
      den<-sapply(1:length(pi6), function(i) dmnorm(cbind(h6[,c(1,2,4)],xx6[,-3]), 
                                                    Mu6[[i]][-c(1)], 
                                                    Sigma6[[i]][-c(1),
                                                                -c(1)])*pi6[i])/
        rowSums(sapply(1:length(pi6), function(i) dmnorm(cbind(h6[,c(1,2,4)],xx6[,-3]), 
                                                         Mu6[[i]][-c(1)], 
                                                         Sigma6[[i]][-c(1),-c(1)])*pi6[i]))
      den1<-ifelse(is.na(den), 0, den)
      y_6[[cc]]<-rowSums(sapply(1:length(pi6), function(i) CondMVN(mean=Mu6[[i]] , sigma=Sigma6[[i]],
                                                                   dependent=c(1), given=c(2:6),X.given=
                                                                     cbind(h6[,c(1,2,4)],xx6[,-3]))$condMean)*(den1))
    ...
    h <- ifelse(is.nan(h),0.1^5,h)
    
    #Metropolis update for the parameters of the marginal of D1
    para.dd1[t,] <- metropolis(h=h, Y=D[,1], X=rbind(xx1,xx2,xx3,xx4,xx5,xx6)[,-3],
    ...
    h[1:(n1+n2+n3),1] <- qnorm(pmin(1-0.1^3,pmax(0.1^3, ptruncnorm(D[1:(n1+n2+n3),1],a=0,b=1,
                                                                   para.dd1[t,ind1][Zm1]+
                                                                     rbind(xx1,xx2,xx3)[,-3]%*%as.matrix(para.dd1[t,ind3]),
    ...
    #Metropolis update for the parameters of the marginal of D2
    para.dd2[t,] <- metropolis(h=h, Y=D[,2], 
                               X=rbind(xx1,xx2,xx3,xx4,xx5,xx6)[,-3],R=R, 
    ...
    h[(n1+n2+n3+1):n,2] <- qnorm(pmin(1-0.1^5,pmax(0.1^5, ptruncnorm(D[(n1+n2+n3+1):n,2],a=0,b=1,
                                                                     para.dd2[t,ind1][Zm2]+
                                                                       rbind(xx4,xx5,xx6)[,-3]%*%as.matrix(para.dd2[t,ind3]),
    ...
    #Metropolis update for the parameters of the marginal of D3
    para.dd3[t,] <- metropolis(h=h, Y=D[,3], X=cbind(rbind(xx1,xx2,xx3,xx4,xx5,xx6),h[,1]),R=R, 
    ...
    #Metropolis update for the parameters of the marginal of D4
    para.dd4[t,] <- metropolis(h=h, Y=D[,4], 
                               X=cbind(rbind(xx1,xx2,xx3,xx4,xx5,xx6),h[,2]),R=R, 
    ...
    #Update the copula correlation matrix R
    ...
    #impute the missing potential compliances
    h1_prop <- h1
    DD1_prop <- DD1
    ...
    rat1 <- dmnorm(h1_prop[,1:2],rep(0,2), R[1:2,1:2], log=TRUE)+
      ...
      log(pmax(0.1^200,rowSums(sapply(1:length(pi1), function(i) dmnorm(cbind(yy1,h1_prop[,1:2],xx1[,-3]), Mu1[[i]], Sigma1[[i]])*pi1[i]))/
                 rowSums(sapply(1:length(pi1), function(i) dmnorm(cbind(h1_prop[,1:2],xx1[,-3]), Mu1[[i]][-1], Sigma1[[i]][-c(1),-c(1)])*pi1[i]))))+
      sapply(1:n1, function(c) log(dtruncnorm(DD1[c,2],a=0,b=1,DD1_prop[c,2], 
                                              sd(DD1[,2])/5)))
    ...
    rat2 <- dmnorm(h1[,1:2],rep(0,2), R[1:2,1:2], log=TRUE)+
      ...
      log(pmax(0.1^200,rowSums(sapply(1:length(pi1), function(i) dmnorm(cbind(yy1,h1[,1:2],xx1[,-3]), Mu1[[i]], Sigma1[[i]])*pi1[i]))/
                 rowSums(sapply(1:length(pi1), function(i) dmnorm(cbind(h1[,1:2],xx1[,-3]), Mu1[[i]][-c(1)], Sigma1[[i]][-c(1),-c(1)])*pi1[i]))))+
      sapply(1:n1, function(c) log(dtruncnorm(DD1_prop[c,2],a=0,b=1,DD1[c,2], 
                                              sd(DD1[,2])/5)))
    ...
    rat1 <- dmnorm(h2_prop,rep(0,4), R, log=TRUE)+
      log(pmax(0.1^100,dtruncnorm(DD2_prop[,2],a=0,b=1,
                                  para.dd2[t,ind1][clus3]+
                                    xx2[,-3]%*%as.matrix(para.dd2[t,ind3]),
                                  sqrt(para.dd2[t,ind2])[clus3])))+
      log(pmax(0.1^300,rowSums(sapply(1:length(pi2), function(i) dmnorm(cbind(yy2,h2_prop[,1:3],xx2[,-3]), Mu2[[i]], Sigma2[[i]])*pi2[i]))/
                 rowSums(sapply(1:length(pi2), function(i) dmnorm(cbind(h2_prop[,1:3],xx2[,-3]), Mu2[[i]][-1], Sigma2[[i]][-c(1),-c(1)])*pi2[i]))))+
    ...
    rat2 <- dmnorm(h2,rep(0,4), R, log=TRUE)+
      log(pmax(0.1^100,dtruncnorm(DD2[,2],a=0,b=1,
                                  para.dd2[t,ind1][clus3]+
                                    xx2[,-3]%*%as.matrix(para.dd2[t,ind3]),
                                  sqrt(para.dd2[t,ind2])[clus3])))+
      log(pmax(0.1^300,rowSums(sapply(1:length(pi2), function(i) dmnorm(cbind(yy2,h2[,1:3],xx2[,-3]), Mu2[[i]], Sigma2[[i]])*pi2[i]))/
                 rowSums(sapply(1:length(pi2), function(i) dmnorm(cbind(h2[,1:3],xx2[,-3]), Mu2[[i]][-c(1)], Sigma2[[i]][-c(1),-c(1)])*pi2[i]))))+
    ...
    rat1 <- dmnorm(h3_prop,rep(0,4), R, log=TRUE)+
      ...
      log(pmax(0.1^100,dtruncnorm(DD3_prop[,3],a=0,b=1,
                                  para.dd3[t,ind1][clus5]+
                                    cbind(xx3,DD3[,1])%*%as.matrix(para.dd3[t,ind13]),
                                  sqrt(para.dd3[t,ind12])[clus5])))+
      log(pmax(0.1^300,rowSums(sapply(1:length(pi3), function(i) dmnorm(cbind(yy3,h3_prop[,1:3],xx3[,-3]), Mu3[[i]], Sigma3[[i]])*pi3[i]))/
                 rowSums(sapply(1:length(pi3), function(i) dmnorm(cbind(h3_prop[,1:3],xx3[,-3]), Mu3[[i]][-1], Sigma3[[i]][-c(1),-c(1)])*pi3[i]))))+
    ...
    rat2 <- dmnorm(h3,rep(0,4), R, log=TRUE)+
      ...
      log(pmax(0.1^100,dtruncnorm(DD3[,3],a=0,b=1,
                                  para.dd3[t,ind1][clus5]+
                                    cbind(xx3,DD3[,1])%*%as.matrix(para.dd3[t,ind13]),
                                  sqrt(para.dd3[t,ind12])[clus5])))+
      log(pmax(0.1^300,rowSums(sapply(1:length(pi3), function(i) dmnorm(cbind(yy3,h3[,1:3],xx3[,-3]), Mu3[[i]], Sigma3[[i]])*pi3[i]))/
                 rowSums(sapply(1:length(pi3), function(i) dmnorm(cbind(h3[,1:3],xx3[,-3]), Mu3[[i]][-c(1)], Sigma3[[i]][-c(1),-c(1)])*pi3[i]))))+
    ...
    h4_prop[,1] <-qnorm(pmin(1-0.1^5,pmax(0.1^100,
                                          ptruncnorm(DD4_prop[,1],a=0,b=1,
                                                     para.dd1[t,ind1][clus6]+
                                                       xx4[,-3]%*%as.matrix(para.dd1[t,ind3]),
    ...
    rat1 <- dmnorm(h4_prop[,1:2],c(0,0), R[1:2,1:2], log=TRUE)+
      ...
      log(pmax(0.1^200,rowSums(sapply(1:length(pi4), function(i) dmnorm(cbind(yy4,h4_prop[,1:2],xx4[,-3]), Mu4[[i]], Sigma4[[i]])*pi4[i]))/
                 rowSums(sapply(1:length(pi4), function(i) dmnorm(cbind(h4_prop[,1:2],xx4[,-3]), Mu4[[i]][-1], Sigma4[[i]][-c(1),-c(1)])*pi4[i]))))+
    ...
    rat2 <- dmnorm(h4[,1:2],c(0,0), R[1:2,1:2], log=TRUE)+
      ...
      log(pmax(0.1^200,rowSums(sapply(1:length(pi4), function(i) dmnorm(cbind(yy4,h4[,1:2],xx4[,-3]), Mu4[[i]], Sigma4[[i]])*pi4[i]))/
                 rowSums(sapply(1:length(pi4), function(i) dmnorm(cbind(h4[,1:2],xx4[,-3]), Mu4[[i]][-c(1)], Sigma4[[i]][-c(1),-c(1)])*pi4[i]))))+
    ...
    rat1 <- dmnorm(h5_prop,rep(0,4), R, log=TRUE)+
      log(pmax(0.1^100,dtruncnorm(DD5_prop[,1],a=0,b=1,
                                  para.dd1[t,ind1][clus8]+
                                    xx5[,-3]%*%as.matrix(para.dd1[t,ind3]),
                                  sqrt(para.dd1[t,ind2])[clus8])))+
      log(pmax(0.1^200,rowSums(sapply(1:length(pi5), function(i) dmnorm(cbind(yy5,h5_prop[,c(1,2,4)],xx5[,-3]), Mu5[[i]], Sigma5[[i]])*pi5[i]))/
                 rowSums(sapply(1:length(pi5), function(i) dmnorm(cbind(h5_prop[,c(1,2,4)],xx5[,-3]), Mu5[[i]][-1], Sigma5[[i]][-c(1),-c(1)])*pi5[i]))))+
    ...
    rat2 <- dmnorm(h5,rep(0,4), R, log=TRUE)+
      log(pmax(0.1^100,dtruncnorm(DD5[,1],a=0,b=1,
                                  para.dd1[t,ind1][clus8]+
                                    xx5[,-3]%*%as.matrix(para.dd1[t,ind3]),
                                  sqrt(para.dd1[t,ind2])[clus8])))+
      log(pmax(0.1^200,rowSums(sapply(1:length(pi5), function(i) dmnorm(cbind(yy5,h5[,c(1,2,4)],xx5[,-3]), Mu5[[i]], Sigma5[[i]])*pi5[i]))/
                 rowSums(sapply(1:length(pi5), function(i) dmnorm(cbind(h5[,c(1,2,4)],xx5[,-3]), Mu5[[i]][-c(1)], Sigma5[[i]][-c(1),-c(1)])*pi5[i]))))+
    ...
    rat1 <- dmnorm(h6_prop,rep(0,4), R, log=TRUE)+
      ...
      log(pmax(0.1^100,dtruncnorm(DD6_prop[,4],a=0,b=1,
                                  para.dd4[t,ind1][clus10]+
                                    cbind(xx6,DD6[,2])%*%as.matrix(para.dd4[t,ind13]),
                                  sqrt(para.dd4[t,ind12])[clus10])))+
      log(pmax(0.1^300,rowSums(sapply(1:length(pi6), function(i) dmnorm(cbind(yy6,h6_prop[,c(1,2,4)],xx6[,-3]), Mu6[[i]], Sigma6[[i]])*pi6[i]))/
                 rowSums(sapply(1:length(pi6), function(i) dmnorm(cbind(h6_prop[,c(1,2,4)],xx6[,-3]), Mu6[[i]][-1], Sigma6[[i]][-c(1),-c(1)])*pi6[i]))))+
    ...
    rat2 <- dmnorm(h6,rep(0,4), R, log=TRUE)+
      ...
      log(pmax(0.1^100,dtruncnorm(DD6[,4],a=0,b=1,para.dd4[t,ind1][clus10]+
                                    cbind(xx6,DD6[,2])%*%as.matrix(para.dd4[t,ind13]),
                                  sqrt(para.dd4[t,ind12])[clus10])))+
      log(pmax(0.1^300,rowSums(sapply(1:length(pi6), function(i) dmnorm(cbind(yy6,h6[,c(1,2,4)],xx6[,-3]), Mu6[[i]], Sigma6[[i]])*pi6[i]))/
                 rowSums(sapply(1:length(pi6), function(i) dmnorm(cbind(h6[,c(1,2,4)],xx6[,-3]), Mu6[[i]][-c(1)], Sigma6[[i]][-c(1),-c(1)])*pi6[i]))))+
