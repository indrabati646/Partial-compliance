...
#log density for Gaussian copula
...
metropolis <- function(h, Y,X, R, w_pre, beta0_pre, beta_pre,sigma_pre, 
  ...
  Cov2 = cov2+diag(ncol(X))
  
  pprop3 <- rmnorm(1,mean=beta_pre, Cov2)
  
  
  # Gaussian variable for the Copula evaluated at the current iterated value
  h.cur1 <- qnorm(pmin(1-0.1^5,pmax(0.1^5, ptruncnorm(Y, a=0,b=1,prop2[zZ]+X%*%prop3, sqrt(sigma_pre[zZ])))))
  
  # Gaussian variable for the Copula evaluated at the proposed value
