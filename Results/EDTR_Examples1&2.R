# Obtaining the EDTRs for Examples 1 and 2
...
source("/Users/indrabatibhattacharya/Documents/biom_code/Response_probability.R")
...
edtr_fun <- function(data){
  ...
  PS_25 <- c(0.25,0.25,0.25,0.25,median(data[,"X1"]),median(data[,"X2"]),
             median(data[,"X3"]))
  PS_00 <- c(1,1,1,1,median(data[,"X1"]),median(data[,"X2"]),median(data[,"X3"]))
  ...
  Sig_seq1 <- matrix(unlist(results_list$ME[[1]]$Sigma1),ncol=6,
                     byrow=T)/length(results_list$ME)
  Sig_seq2 <- matrix(unlist(results_list$ME[[1]]$Sigma2),ncol=7,
                     byrow=T)/length(results_list$ME)
  Sig_seq3 <- matrix(unlist(results_list$ME[[1]]$Sigma3),ncol=7,
                     byrow=T)/length(results_list$ME)
  Sig_seq4 <-matrix(unlist(results_list$ME[[1]]$Sigma4),ncol=6,
                    byrow=T)/length(results_list$ME)
  Sig_seq5 <- matrix(unlist(results_list$ME[[1]]$Sigma5),ncol=7,
                     byrow=T)/length(results_list$ME)
  Sig_seq6 <- matrix(unlist(results_list$ME[[1]]$Sigma6),ncol=7,
                     byrow=T)/length(results_list$ME)
  ...
  b15ind1 <- which(colnames(res_prob$samples)=="beta5[1]")
  b15ind2 <- which(colnames(res_prob$samples)=="beta5[5]")
  ...
  b25ind1 <- which(colnames(res_prob$samples1)=="beta5[1]")
  b25ind2 <- which(colnames(res_prob$samples1)=="beta5[5]")
  ...
  y_seq1 <- condMVN(Mu_seq1,Sig_seq1,dependent.ind=c(1),
                    given.ind=c(2:6),X.given = PS_75[-c(3,4)])
  y_seq2 <- condMVN(Mu_seq2,Sig_seq2,dependent.ind=c(1),given.ind=c(2:7),
                    X.given = PS_75[-4])
  y_seq3 <- condMVN(Mu_seq3,Sig_seq3,dependent.ind=c(1),given.ind=c(2:7),
                    X.given = PS_75[-4])
  y_seq4 <- condMVN(Mu_seq4,Sig_seq4,dependent.ind=c(1),
                    given.ind=c(2:6),X.given = PS_75[-c(3,4)])
  y_seq5 <- condMVN(Mu_seq5,Sig_seq5,dependent.ind=c(1),given.ind=c(2:7),
                    X.given = PS_75[-3])
  y_seq6 <- condMVN(Mu_seq6,Sig_seq6,dependent.ind=c(1),given.ind=c(2:7),
                    X.given = PS_75[-3])
  ...
  for(i in 1:length(results_list$ME)){
    for(j in 1:5){
      prob1[i,j] <- res_prob$samples[i,w1ind1+j-1]*(res_prob$samples[i,a1ind1+j-1]+
                                                   ...
                                                   res_prob$samples[i,b15ind1+j-1]*PS_75[7])
    }
  }
  ...
  for(i in 1:length(results_list$ME)){
    for(j in 1:5){
      prob2[i,j] <- res_prob$samples1[i,w2ind1+j-1]*(res_prob$samples1[i,a2ind1+j-1]+
                                            ...
                                            res_prob$samples1[i,b25ind1+j-1]*PS_75[7])
    }
  }
  ...
  y_seq1 <- condMVN(Mu_seq1,Sig_seq1,dependent.ind=c(1),
                    given.ind=c(2:6),X.given = PS_50[-c(3,4)])
  y_seq2 <- condMVN(Mu_seq2,Sig_seq2,dependent.ind=c(1),given.ind=c(2:7),
                    X.given = PS_50[-4])
  y_seq3 <- condMVN(Mu_seq3,Sig_seq3,dependent.ind=c(1),given.ind=c(2:7),
                    X.given = PS_50[-4])
  ...
  for(i in 1:length(results_list$ME)){
    for(j in 1:5){
      prob1[i,j] <- res_prob$samples[i,w1ind1+j-1]*(res_prob$samples[i,a1ind1+j-1]+
                                                   ...
                                                   res_prob$samples[i,b15ind1+j-1]*PS_50[7])
    }
  }
  ...
  for(i in 1:length(results_list$ME)){
    for(j in 1:5){
      prob2[i,j] <- res_prob$samples1[i,w2ind1+j-1]*(res_prob$samples1[i,a2ind1+j-1]+
                                            ...
                                            res_prob$samples1[i,b25ind1+j-1]*PS_50[7])
    }
  }
...
y_seq1 <- condMVN(Mu_seq1,Sig_seq1,dependent.ind=c(1),
                  given.ind=c(2:6),X.given = PS_25[-c(3,4)])
y_seq2 <- condMVN(Mu_seq2,Sig_seq2,dependent.ind=c(1),given.ind=c(2:7),
                  X.given = PS_25[-4])
y_seq3 <- condMVN(Mu_seq3,Sig_seq3,dependent.ind=c(1),given.ind=c(2:7),
                  X.given = PS_25[-4])
...
for(i in 1:length(results_list$ME)){
  for(j in 1:5){
    prob1[i,j] <- res_prob$samples[i,w1ind1+j-1]*(res_prob$samples[i,a1ind1+j-1]+
                                                 ...
                                                 res_prob$samples[i,b15ind1+j-1]*PS_25[7])
  }
}
...
for(i in 1:length(results_list$ME)){
  for(j in 1:5){
    prob2[i,j] <- res_prob$samples1[i,w2ind1+j-1]*(res_prob$samples1[i,a2ind1+j-1]+
                                                   ...
                                                   res_prob$samples1[i,b25ind1+j-1]*PS_25[7])
  }
}
...
y_seq1 <- condMVN(Mu_seq1,Sig_seq1,dependent.ind=c(1),
                  given.ind=c(2:6),X.given = PS_00[-c(3,4)])
y_seq2 <- condMVN(Mu_seq2,Sig_seq2,dependent.ind=c(1),given.ind=c(2:7),
                  X.given = PS_00[-4])
y_seq3 <- condMVN(Mu_seq3,Sig_seq3,dependent.ind=c(1),given.ind=c(2:7),
                  X.given = PS_00[-4])
...
for(i in 1:length(results_list$ME)){
  for(j in 1:5){
    prob1[i,j] <- res_prob$samples[i,w1ind1+j-1]*(res_prob$samples[i,a1ind1+j-1]+
                                                 ...
                                                 res_prob$samples[i,b15ind1+j-1]*PS_00[7])
  }
}
...
for(i in 1:length(results_list$ME)){
  for(j in 1:5){
    prob2[i,j] <- res_prob$samples1[i,w2ind1+j-1]*(res_prob$samples1[i,a2ind1+j-1]+
                                                   ...
                                                   res_prob$samples1[i,b25ind1+j-1]*PS_00[7])
  }
}
...
return(list(EDTR_00_mean,EDTR_75_mean,EDTR_50_mean,EDTR_25_mean,bst_00,bst_75,bst_50,bst_25))
}
...
#True EDTRs
tdtr_00 <- tdtr_75 <- tdtr_50 <- tdtr_25<-c()
tdtr_00[1] <- 0.5*(0.7+0.6*1+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                   0.2+0.7*1-0.9*median(data[,"X1"])+1.5*median(data[,"X2"]))
tdtr_00[2] <- 0.5*(0.7+0.6*1+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                   0.2+0.6*1+0.9*median(data[,"X1"])+median(data[,"X3"]))
tdtr_00[3] <- 0.5*(0.7+0.6*1+0.6*1+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                   0.3+0.6*1-0.5*median(data[,"X2"])+1.5*median(data[,"X3"]))
tdtr_00[4] <- 0.5*(0.7+0.6*1+0.6*1+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                   0.3+0.8*1+0.7*1-0.5*median(data[,"X2"])+0.9*median(data[,"X3"]))

tdtr_75[1] <- 0.5*(0.7+0.6*0.75+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                     0.2+0.7*0.75-0.9*median(data[,"X1"])+1.5*median(data[,"X2"]))
tdtr_75[2] <- 0.5*(0.7+0.6*0.75+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                     0.2+0.6*0.75+0.9*median(data[,"X1"])+median(data[,"X3"]))
tdtr_75[3] <- 0.5*(0.7+0.6*0.75+0.6*0.75+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                     0.3+0.6*0.75-0.5*median(data[,"X2"])+1.5*median(data[,"X3"]))
tdtr_75[4] <- 0.5*(0.7+0.6*0.75+0.6*0.75+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                     0.3+0.8*0.75+0.7*0.75-0.5*median(data[,"X2"])+0.9*median(data[,"X3"]))

tdtr_50[1] <- 0.5*(0.7+0.6*0.50+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                     0.2+0.7*0.50-0.9*median(data[,"X1"])+1.5*median(data[,"X2"]))
tdtr_50[2] <- 0.5*(0.7+0.6*0.50+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                     0.2+0.6*0.50+0.9*median(data[,"X1"])+median(data[,"X3"]))
tdtr_50[3] <- 0.5*(0.7+0.6*0.50+0.6*0.50+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                     0.3+0.6*0.50-0.5*median(data[,"X2"])+1.5*median(data[,"X3"]))
tdtr_50[4] <- 0.5*(0.7+0.6*0.50+0.6*0.50+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                     0.3+0.8*0.50+0.7*0.50-0.5*median(data[,"X2"])+0.9*median(data[,"X3"]))

tdtr_25[1] <- 0.5*(0.7+0.6*0.25+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                     0.2+0.7*0.25-0.9*median(data[,"X1"])+1.5*median(data[,"X2"]))
tdtr_25[2] <- 0.5*(0.7+0.6*0.25+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                     0.2+0.6*0.25+0.9*median(data[,"X1"])+median(data[,"X3"]))
tdtr_25[3] <- 0.5*(0.7+0.6*0.25+0.6*0.25+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                     0.3+0.6*0.25-0.5*median(data[,"X2"])+1.5*median(data[,"X3"]))
tdtr_25[4] <- 0.5*(0.7+0.6*0.25+0.6*0.25+0.8*median(data[,"X1"])-0.2*median(data[,"X2"])+
                     0.3+0.8*0.25+0.7*0.25-0.5*median(data[,"X2"])+0.9*median(data[,"X3"]))
