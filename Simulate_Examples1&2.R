...
ComplianceSimulate <- function(n, n_sim){
  ...
  for (i in 1:n_sim){
    ...
    cop <- normalCopula(param=c(0.2,0.2,0.2), dim = 3, dispstr = "un")
    
    D <- matrix(NA,nrow=n,ncol=3)
    
    
    for (j in 1:n) {
      mvd <- mvdc(copula=cop, margins=c("truncnorm","truncnorm","truncnorm"),
    ...
    y1[a1==1&s==1] = 0.7+0.6*D1[a1==1&s==1]+0.8*(X1[a1==1&s==1])-0.2*(X2[a1==1&s==1])
    
    y1[a1==1&s==0&a2==1] = 0.2+0.7*D1[a1==1&s==0&a2==1]-0.9*(X1[a1==1&s==0&a2==1])+1.5*(X3[a1==1&s==0&a2==1])
    
    y1[a1==1&s==0&a2==-1] = 0.2+0.6*D1[a1==1&s==0&a2==-1]+0.9*(X1[a1==1&s==0&a2==-1])+X3[a1==1&s==0&a2==-1]
    
    y1[a1==-1&s==1] = 0.7+0.6*D1[a1==-1&s==1]+0.6*D2[a1==-1&s==1]+0.8*(X1[a1==-1&s==1])-0.2*(X2[a1==-1&s==1])
    
    y1[a1==-1&s==0&a2==1] = 0.3+0.6*D2[a1==-1&s==0&a2==1]-0.5*(X2[a1==-1&s==0&a2==1])+1.5*X3[a1==-1&s==0&a2==1]
    
    y1[a1==-1&s==0&a2==-1] = 0.3+0.8*D1[a1==-1&s==0&a2==-1]+0.7*D2[a1==-1&s==0&a2==-1]-0.5*(X2[a1==-1&s==0&a2==-1])+0.9*X3[a1==-1&s==0&a2==-1]
    
    
    #Add noise to form outcomes on the scale of the coefficients
    #y <- y1 + rnorm(n,0,0.1) #If Example 1
   y <- y1+ rbeta(n,0.5,0.5)-0.5 # If Example 2
    sim_list[[i]] <- data.frame(a1,a2,s,D1,D2,D3,y,y1,X1,X2,X3)
